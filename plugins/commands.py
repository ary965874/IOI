...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton('✪ Sᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ', url="https://t.me/AX_FILMZ"),
                    InlineKeyboardButton('⌬ Mᴏᴠɪᴇ Gʀᴏᴜᴘ', url="https://t.me/AX_FILMZ")
                ],[
        ...
        for file in files:
            ...
            if not await check_verification(client, message.from_user.id) and VERIFY == True:
                btn = [
                    ...
                    InlineKeyboardButton("❓❗ How to Verify ❗❓", url="https://telegram.me/AX_FILMZ/106")
                ]
                ]
                await message.reply_text(
    ...
    if not files_:
        ...
        try:
            ...
            title = '@AX_FILMZ  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@AX_FILMZ ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
